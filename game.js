...
function stopGame() {
   ...
   clearTimeout(addShapeTimer);
   canvas.clear();
   //gameStop = true;
}

ractive.observe('time', function( newValue, oldValue, keypath) {
...
function addShape() {
    ...
    if(shapes.length < 5) {
        addShapeTimer = setTimeout(addShape,2000);
    }
    else if(shapes.length >= 5 && shapes.length <= 10){
        addShapeTimer = setTimeout(addShape, 1500);
    }
    else if(shapes.length > 10){
        addShapeTimer = setTimeout(addShape, 3000);
    }
}
